...
const LedgerReport = (props: Props) => {
  ...
  const handleReport = (start: any, end: any, pids?: any, uids?: any) => {
    ...
    POSTMethod('/report/income_expense/user', request).then(response => {
      setTotalAmount(response.data?.Profit);
      setCreditAmount(response.data["Credit Amount"]);
      setDebitAmount(response.data["Debit Amount"])
      setCollectionData(response.data["User IncomeExpense List"])
      console.log(JSON.stringify(response.data["User IncomeExpense List"]));
    })
  ...
  const handleFromDateConfirm = (date: any) => {
    ...
    if (toDate) {
      handleReport(moment(date).format('DD-MM-YYYY'), toDate, selectedPropertyIds, unitIds)
    }
  }
  ...
  const handleToDateConfirm = (date: any) => {
    ...
    handleReport(fromDate, moment(date).format('DD-MM-YYYY'), selectedPropertyIds, unitIds)
  }
...

            {
             collectionData&& collectionData.length > 0
                ...
                <>
                  ...
                  <ListHeader
                    data={["DESCRIPTION", "DATE", "CREDIT", "DEBIT"]}
                  />

                  <FlatList
                    data={collectionData}
                    keyExtractor={(item) => `collections-${item.id}`}
                    showsVerticalScrollIndicator={false}
                    bounces={false}
                    ListFooterComponent={
                      <View style={[GlobalStyles.rowCenter, styles.listHeaderContainer]} >
                        <View style={{ ...styles.cellContainer, width: '50%' }} >
                          <Text style={{ ...GlobalStyles.txtR14DG, width: '100%' }} >Total</Text>
                        </View>
                        <View style={styles.cellContainer} >
                          <Text style={{ ...GlobalStyles.txtR14DG, width: '100%' }} >{creditAmount}</Text>
                        </View>
                        <View style={styles.cellContainer} >
                          <Text style={{ ...GlobalStyles.txtR14DG, width: '100%' }} >{debitAmount}</Text>
                        </View>
                      </View>}
                        ...
                        { backgroundColor: index % 2 == 1 ? COLORS.lightWhite : COLORS.white }]} >
                          ...
                          <View style={styles.cellContainer} >
                            <Text style={{ ...GlobalStyles.txtR10DG, width: '100%' }}
                              numberOfLines={2} >{item?.date}</Text>
                          </View>
                          <View style={styles.cellContainer} >
                            <Text style={{ ...GlobalStyles.txtR10DG, width: '100%' }}
                              numberOfLines={2} adjustsFontSizeToFit>{item.credit}</Text>
                          </View>
                          <View style={styles.cellContainer} >
                            <Text style={{ ...GlobalStyles.txtR10DG, width: '100%' }}
                              numberOfLines={2} adjustsFontSizeToFit>{item?.debit}</Text>
                          </View>
                        </View>
                      )
                    }}
