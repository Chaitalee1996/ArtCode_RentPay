...
const DepositeReport= (props: Props) => {

  const [depositeData, setDepositeData] = useState<any>([]);
  ...
  const handleReport = (start: any, end: any, pids?: any, uids?: any) => {
    ...
    POSTMethod('/report/deposits', request).then(response => {
      setTotalAmount(response.data?.totalDeposite);
      setDepositeData(response.data["Deposits List"])
      console.log(JSON.stringify(response.data["Deposits List"]));
    })
  ...
  return (
    <CustomContainer>
      ...
      {/* header */}
      <CustomHeader title='Deposite Report' />
...

            {
              depositeData.length > 0
                ...
                <>
                  ...
                  <ListHeader
                    data={["TENANTS", "PROPERTY", "DATE", "DEPOSITE"]}
                  />

                  <FlatList
                    data={depositeData}
                    keyExtractor={(item) => `deposite-${item.id}`}
                    ...
                    renderItem={({ item, index }) => {
                      return (
                        ...
                        { backgroundColor: index % 2 == 1 ? COLORS.lightWhite : COLORS.white }]} >
                          ...
                          <View style={styles.cellContainer} >
                            <Text style={{ ...GlobalStyles.txtR10DG, width: '100%' }}
                              numberOfLines={2} adjustsFontSizeToFit>{moment(item?.possessionDate).format("DD-MM-yyyy")}</Text>
                          </View>
                          <View style={styles.cellContainer} >
                            <Text style={{ ...GlobalStyles.txtR10DG, width: '100%' }}
                              numberOfLines={2} adjustsFontSizeToFit>{item?.deposite}</Text>
                          </View>
                        </View>
                      )
                    }}
