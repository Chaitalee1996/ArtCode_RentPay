...
import { FilterButton, ListHeader, NoReportDataComponent, PropertyUnitFilter, TotalAndExportComponent } from './components'
...
const CollectionReport = (props: Props) => {
  ...
  const handleReport = (start: any, end: any, pids?: any, uids?: any) => {
    ...
    POSTMethod('/report/collection', request).then(response => {
      setTotalAmount(response.data?.TotalPaidAmount);
      setCollectionData(response.data["Collection List"])
      console.log(JSON.stringify(response.data["Collection List"]));
    })
  ...
  return (
    <CustomContainer>
      ...
      {/* header */}
      <CustomHeader title='Collection Report' />
...

            {
              collectionData.length > 0
                ...
                <>
                  ...
                  <ListHeader
                    data={["TENANTS", "PROPERTIES", "DATE", "COLLECTION"]}
                  />
                        ...
                        { backgroundColor: index % 2 == 1 ? COLORS.lightWhite : COLORS.white }]} >
                          <View style={{ ...styles.cellContainer, }} >
                            <Text style={{ ...GlobalStyles.txtR10DG, width: '98%' }}
                              numberOfLines={2} >{item.tenantName}</Text>
                          </View>
                          <View style={styles.cellContainer} >
                          ...
                          <View style={styles.cellContainer} >
                            <Text style={{ ...GlobalStyles.txtR10DG, width: '100%' }}
                              numberOfLines={2} adjustsFontSizeToFit>{moment(item?.collectionDate).format("DD-MM-yyyy")}</Text>
                          </View>
                          <View style={styles.cellContainer} >
                            <Text style={{ ...GlobalStyles.txtR10DG, width: '100%' }}
                              numberOfLines={2} adjustsFontSizeToFit>{item?.paidAmount}</Text>
                          </View>
                        </View>
                      )
                    }}
